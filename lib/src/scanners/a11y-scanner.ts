...
const FORM_ELEMENTS = new Set([
  ...
  // 组件库常见命名
  'Input', 'Select', 'Textarea', 'TextField', 'AutoComplete', 'DatePicker',
  'TimePicker', 'Search', 'Password', 'Checkbox', 'Radio', 'Switch', 'Slider',
  'Rate', 'Upload', 'Cascader', 'TreeSelect', 'Mention',
]);

/** 非交互元素（可点击时需要语义化） */
const NON_INTERACTIVE_TAGS = new Set([
  'div', 'span', 'i', 'em', 'strong', 'b', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
...
const INTERACTIVE_EVENTS = new Set([
  ...
  'onMouseLeave', 'onMouseOver', 'onMouseOut', 'onTouchStart', 'onTouchEnd',
      ...
      traverse(ast, {
        JSXAttribute(path) {
          ...
          if (name.startsWith('aria-')) {
            ...
            if (name === 'aria-hidden' && path.node.value?.type === 'StringLiteral' && path.node.value.value === 'true') {
...
/** 检查表单元素是否有 label 关联 */
function checkFormLabel(node: any): { hasLabel: boolean; line: number; column: number; labelAttrs: string[] } {
  const { line, column } = node.loc?.start || { line: 0, column: 0 };
...
function checkRole(node: any): { hasRole: boolean; roleValue: string | null; hasTabIndex: boolean; line: number; column: number } {
  const { line, column } = node.loc?.start || { line: 0, column: 0 };
  let hasRole = false;
    ...
    if (attr.name.name === 'style' && attr.value?.type === 'JSXExpressionContainer') {
      ...
      if (expr.type === 'ObjectExpression') {
        for (const prop of expr.properties) {
          ...
          if ((key === 'backgroundColor' || key === 'background-color') && prop.value.type === 'StringLiteral') {
            bg = prop.value.value;
          }
        }
      }
