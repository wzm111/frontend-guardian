...
import { createEngine, i18nRules, performanceRules, a11yRules, securityRules, namingRules, crossFileRules, componentRules, hooksRules, platformRules } from '../dist/index.js';
...
Options:
  --module <name>      扫描模块: i18n | performance | a11y | security | naming | cross-file | component | hooks | platform
...
async function main() {
  ...
  switch (options.module) {
    case 'i18n':
      ...
      break;
    case 'performance':
      engine.registerAll(performanceRules);
      break;
    case 'a11y':
      engine.registerAll(a11yRules);
      break;
    case 'security':
      engine.registerAll(securityRules);
      break;
    case 'naming':
      engine.registerAll(namingRules);
      break;
    case 'cross-file':
      engine.registerAll(crossFileRules);
      break;
    case 'component':
      engine.registerAll(componentRules);
      break;
    case 'hooks':
      engine.registerAll(hooksRules);
      break;
    case 'platform':
      engine.registerAll(platformRules);
      break;
    default:
  ...
  try {
    ...
    if (options.fix) {
      ...
      if (fixableIssues.length > 0) {
        ...
        const fixResult = engine.applyFixes(fixableIssues);
        if (fixResult.filesModified.length > 0) {
          console.log(pc.green(`   ✅ 已修复 ${fixResult.fixedCount} 个问题（${fixResult.filesModified.length} 个文件）`));
        ...
        console.log(pc.gray('   ℹ️ 没有可自动修复的问题'));
      }
    }
