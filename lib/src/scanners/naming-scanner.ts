    ...
    execute(context: RuleContext): Issue[] {
      ...
      traverse(ast, {
        ...
        FunctionDeclaration(path) {
          ...
          const isComponent =
            config.allowPascalCaseComponents && isLikelyReactComponent(path.node, name);
        ...
        VariableDeclarator(path) {
          ...
          // 检测是否是 React 组件
          const isComponent =
            config.allowPascalCaseComponents && isLikelyReactComponent(init, name);
  ...
  },
  {
    ...
    execute(context: RuleContext): Issue[] {
      ...
      if (
        ...
        config.folderNameCase === "kebab-case" &&
        !isKebabCase(dirName) &&
        !isCamelCase(dirName)
      ) {
        // 只检查直接父目录（避免检查太深层）
...
function isJSXExpression(node: any): boolean {
  ...
  return (
    ...
    (node.type === "LogicalExpression" &&
      (isJSXExpression(node.left) || isJSXExpression(node.right))) ||
