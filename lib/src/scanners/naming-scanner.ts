      ...
      traverse(ast, {
        TSEnumDeclaration(path) {
          ...
          for (const member of path.node.members) {
            ...
            const memberName = member.id.type === 'Identifier' ? member.id.name :
                              member.id.type === 'StringLiteral' ? member.id.value : null;
  ...
  },
  {
    ...
    execute(context: RuleContext): Issue[] {
      ...
      if (
        ...
        // 跳过配置文件和入口文件
      } else if (config.fileNameCase === 'kebab-case' && !isKebabCase(nameWithoutExt) && !isCamelCase(nameWithoutExt)) {
...
function isJSXExpression(node: any): boolean {
  ...
  return node.type === 'JSXElement' ||
         ...
         node.type === 'JSXText' ||
         (node.type === 'ConditionalExpression' && (isJSXExpression(node.consequent) || isJSXExpression(node.alternate))) ||
