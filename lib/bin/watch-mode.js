...
/**
 * v3.15.0: 根据变更文件推断需要扫描的模块
 */
export function resolveModulesForFiles(files) {
    ...
    for (const file of files) {
        ...
        const ext = extIndex >= 0 ? file.slice(extIndex).toLowerCase() : "";

        if (rel.includes("test") || rel.includes("spec") || rel.includes("e2e")) {
            modules.add("e2e");
        }
        if (rel.includes("i18n") || rel.includes("locale") || rel.includes("lang") || rel.includes("messages")) {
            modules.add("i18n");
        }
        if (rel.includes("hook") || rel.includes("composable")) {
            modules.add("hooks");
        }
    ...
    if (hasSource) {
        ...
        modules.add("performance");
        modules.add("a11y");
        modules.add("cross-file");
    }

    const result = Array.from(modules);
    return result.length > 0 ? result : MODULES;
}
 ...
 * @param {Function} runSingleModuleFn - 单模块扫描函数 (options, cacheInstance) => Promise<void>（可选）
 * @param {Object} engine - RuleEngine 实例（可选，用于自定义规则热重载）
 */
export async function runWatchMode(options, scanFn, cacheInstance, runSingleModuleFn, engine) {
    ...
    function handleChange(filePath) {
        ...
        debounceTimer = setTimeout(async () => {
            ...
            try {
                // v3.15.0: Watch 模式增量优化：只扫描变更文件涉及的模块
                const targetModules =
                    options.module === "all" && runSingleModuleFn
                        ? resolveModulesForFiles(files)
                        : [options.module];

                if (options.module === "all" && targetModules.length < MODULES.length && runSingleModuleFn) {
                    console.log(pc.cyan(`🎯 增量模块: ${targetModules.join(", ")}`));
                    console.log("");
                    for (const mod of targetModules) {
                        ...
                        await runSingleModuleFn(singleOptions, cacheInstance);
                        console.log("");
                    }
                } else {
                    const watchOptions = {
    ...
    // v3.17.0: 监听自定义规则文件变更，实现热重载
    if (engine && engine.config && engine.config.customRules && engine.config.customRules.length > 0) {
        for (const customRule of engine.config.customRules) {
            const customPath = resolve(projectDir, customRule.path);
            if (!existsSync(customPath)) continue;
            try {
                const customWatcher = watch(customPath, (eventType) => {
                    if (eventType !== "change") return;
                    console.log(pc.cyan(`\n🔄 自定义规则变更 detected: ${relative(projectDir, customPath)}`));
                    const ok = engine.reloadCustomRule(customRule.path);
                    if (ok) {
                        console.log(pc.green("   ✅ 规则已重新加载，准备重新扫描...\n"));
                    } else {
