...
const REACTIVE_NAMES = new Set([
  'state', 'count', 'value', 'item', 'index', 'id', 'name',
  'isOpen', 'isVisible', 'isLoading', 'error', 'result', 'response', 'data',
  'list', 'form', 'query', 'params', 'user', 'current',
]);
            ...
            if (body.type === 'BlockStatement') {
              ...
              for (const name of REACTIVE_NAMES) {
                ...
                if (regex.test(bodyText) && depsArray.type === 'ArrayExpression') {
                  const inDeps = depsArray.elements.some((el: any) =>
                    el?.type === 'Identifier' && el.name === name
                  );
                  if (!inDeps) {
                    issues.push({
                      ruleId: 'hooks-effect-deps',
    ...
    execute(context: RuleContext): Issue[] {
      ...
      traverse(ast, {
        CallExpression(path) {
          ...
          const useEffectPath = path.findParent((p: any) =>
            p.isCallExpression() && p.node.callee?.type === 'Identifier' && p.node.callee.name === 'useEffect'
          );
