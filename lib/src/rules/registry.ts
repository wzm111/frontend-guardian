  ...
  loadCustomRule(filePath: string, projectDir?: string): boolean {
    ...
    try {
      ...
      if (!rule || !rule.id || typeof rule.execute !== "function") {
        console.warn(
          pc.yellow(
            `⚠️ RuleRegistry: 自定义规则文件格式不正确（需导出 Rule 对象）: ${absolutePath}`,
          ),
        );
        return false;
      }
  ...
  filterRules(options?: {
    ...
    return this.getActiveRules().filter((rule) => {
      ...
      if (
        options?.framework &&
        rule.frameworks &&
        !rule.frameworks.includes(options.framework as any)
      )
        return false;
      if (options?.platform && rule.platforms && !rule.platforms.includes(options.platform as any))
        return false;
      if (
