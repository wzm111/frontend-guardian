    ...
    execute(context: RuleContext): Issue[] {
      ...
      while ((match = base64Regex.exec(source)) !== null) {
        ...
        issues.push({
          ...
          description:
            "检测到较大的 base64 图片内联，会增加包体积。建议改为网络图片或放到 static 目录",
          severity: "warning",
          file: context.filePath,
          line,
          column: 1,
        ...
        if (/position\s*:\s*fixed|fixed\s*:\s*bottom/i.test(source)) {
          if (!/safe-area-inset|env\(/i.test(source)) {
            issues.push({
              ...
              description:
                "检测到 position: fixed 布局，建议添加 safe-area-inset-bottom 适配刘海屏/全面屏",
              severity: "warning",
              file: context.filePath,
              line: 1,
              column: 1,
    ...
    execute(context: RuleContext): Issue[] {
      ...
      if (
        ...
        issues.push({
          ...
          title: "未检测到响应式断点配置",
          description:
            "PC/H5 项目建议配置响应式断点（@media query 或 JS hook），以适配不同屏幕尺寸",
          severity: "suggestion",
          file: context.filePath,
          line: 1,
          column: 1,
