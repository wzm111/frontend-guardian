...
Options:
  ...
  --screenshot-selector <selector>  元素级截图选择器（配合 --page-health）
  --max-diff-pixels <n>  视觉回归最大差异像素数（默认 100，配合 --page-health）
  --max-diff-pixel-ratio <n>  视觉回归最大差异比例（默认 0.01，配合 --page-health）
  --mask-selectors <list>  额外遮罩选择器，逗号分隔（配合 --page-health）
  --no-mask            禁用动态内容遮罩（配合 --page-health）
  ...
  --miniprogram-performance  启用小程序性能采集：构建指标 + setData 静态分析 + 运行时阈值（配合 --mini-program）
  --miniprogram-performance-threshold-startup <ms>   启动时间阈值（默认 2000ms）
  --miniprogram-performance-threshold-fps <n>        最低 FPS 阈值（默认 30）
  --miniprogram-performance-threshold-setdata-count <n>  单页 setData 次数阈值（默认 50）
  --miniprogram-performance-threshold-setdata-payload <bytes>  setData 平均负载阈值（默认 10240）
  --miniprogram-performance-threshold-package-size <bytes>    主包/分包体积阈值（默认 2097152）
  --miniprogram-performance-threshold-page-size <bytes>       单页体积阈值（默认 512000）
  --miniprogram-cross-platform-diff              启用跨平台截图差异对比（需配合 --mini-program all 与 --miniprogram-screenshot）
  --miniprogram-diff-mode <reference|pairwise>   跨平台对比模式（默认 reference）
  --miniprogram-diff-reference <wechat|alipay|douyin>  参考平台（默认 wechat，仅 reference 模式）
  --miniprogram-diff-pages <list>                要对比的页面，逗号分隔（默认取 app.json pages 前 10 个）
  --miniprogram-diff-max-pages <n>               最大对比页面数（默认 10）
  --miniprogram-diff-threshold-pixels <n>        差异像素阈值（默认 100）
  --miniprogram-diff-threshold-ratio <n>         差异比例阈值（默认 0.01）
  --mobile                          启用移动端测试模式（Maestro / Appium）（v4.0.0）
  --maestro                         等价于 --mobile-tool maestro（v4.0.0）
  --appium                          等价于 --mobile-tool appium（v4.0.0）
  --mobile-tool <auto|maestro|appium>  选择移动端工具（默认 auto）（v4.0.0）
  --mobile-page-health              移动端页面健康检查：截图/白屏/崩溃/ANR（v4.0.0）
  --mobile-app-id <id>              移动应用 bundle id / appId（v4.0.0）
  --mobile-routes <list>            要检查的页面路由/deeplink 路径，逗号分隔（v4.0.0）
  --mobile-screenshot-dir <dir>     移动端截图目录（默认 .frontend-guardian/screenshots/mobile）（v4.0.0）
  --mobile-update-baseline          更新移动端截图基线（v4.0.0）
  --mobile-concurrency <n>          移动端页面健康检查并发数（默认 1）（v4.0.0）
  --mobile-timeout <ms>             单页面检查超时（默认 60000）（v4.0.0）
...
Examples:
  ...
  fg-core ./wechat-mini-program --mini-program auto
  fg-core ./wechat-mini-program --mini-program wechat --miniprogram-screenshot
  fg-core ./multi-platform-mini-program --mini-program all --miniprogram-screenshot
  fg-core ./wechat-mini-program --mini-program wechat --miniprogram-performance
  fg-core ./multi-platform-mini-program --mini-program all --miniprogram-screenshot --miniprogram-cross-platform-diff
  fg-core ./multi-platform-mini-program --mini-program all --miniprogram-screenshot --miniprogram-cross-platform-diff \
    --miniprogram-diff-mode pairwise --miniprogram-diff-threshold-pixels 50
  fg-core ./my-app --mobile --maestro
  fg-core ./my-app --mobile --appium --json
  fg-core ./my-app --mobile-page-health --mobile-app-id com.example.app --mobile-routes home,profile
`);
}
...
async function main() {
    ...
    const options = {
        ...
        noMask: false,
        maskSelectors: undefined,
        pageHealthMetrics: false,
        ...
        miniProgramScreenshot: false,
        miniProgramUpdateBaseline: false,
        miniProgramPerformance: false,
        miniProgramPerformanceThresholds: {},
        // v3.12.0
        miniProgramCrossPlatformDiff: false,
        miniProgramDiffMode: undefined,
        miniProgramDiffReference: undefined,
        miniProgramDiffPages: undefined,
        miniProgramDiffMaxPages: undefined,
        miniProgramDiffThresholdPixels: undefined,
        miniProgramDiffThresholdRatio: undefined,
        // v4.0.0
        mobile: false,
        mobileTool: "auto",
        mobilePageHealth: false,
        mobileAppId: undefined,
        mobileRoutes: undefined,
        mobileScreenshot: true,
        mobileScreenshotDir: undefined,
        mobileUpdateBaseline: false,
        mobileConcurrency: 1,
        mobileTimeout: 60000,
        buildIndex: false,
        watchIndex: false,
    ...
    for (let i = 0; i < args.length; i++) {
        switch (args[i]) {
            ...
            case "--serve":
                // --serve 单独使用：启动本地看板服务
                // --serve <cmd>：为 page-health 启动 dev server
            ...
            case "--screenshot-selector":
                options.screenshotSelector = args[++i];
                break;
            case "--max-diff-pixels":
                options.maxDiffPixels = parseInt(args[++i], 10);
                break;
            case "--max-diff-pixel-ratio":
                options.maxDiffPixelRatio = parseFloat(args[++i]);
                break;
            case "--mask-selectors":
                options.maskSelectors = args[++i].split(",");
                break;
            case "--no-mask":
                options.noMask = true;
                break;
            case "--page-health-metrics":
                options.pageHealthMetrics = true;
                break;
            case "--a11y":
                options.a11y = true;
                break;
            ...
            case "--mini-program": {
                ...
                if (next && !next.startsWith("-")) {
                    options.miniProgram = args[++i];
                } else {
                    options.miniProgram = "auto";
                }
                break;
            }
            case "--miniprogram-screenshot":
                options.miniProgramScreenshot = true;
                break;
            case "--miniprogram-update-baseline":
                options.miniProgramUpdateBaseline = true;
                break;
            case "--miniprogram-performance":
                options.miniProgramPerformance = true;
                break;
            case "--miniprogram-performance-threshold-startup":
                options.miniProgramPerformanceThresholds.startup = parseInt(args[++i], 10);
                break;
            case "--miniprogram-performance-threshold-fps":
                options.miniProgramPerformanceThresholds.fps = parseInt(args[++i], 10);
                break;
            case "--miniprogram-performance-threshold-setdata-count":
                options.miniProgramPerformanceThresholds.setDataCount = parseInt(args[++i], 10);
                break;
            case "--miniprogram-performance-threshold-setdata-payload":
                options.miniProgramPerformanceThresholds.setDataPayloadBytes = parseInt(args[++i], 10);
                break;
            case "--miniprogram-performance-threshold-package-size":
                options.miniProgramPerformanceThresholds.packageSize = parseInt(args[++i], 10);
                break;
            case "--miniprogram-performance-threshold-page-size":
                options.miniProgramPerformanceThresholds.pageSize = parseInt(args[++i], 10);
                break;
            case "--miniprogram-cross-platform-diff":
                options.miniProgramCrossPlatformDiff = true;
                break;
            case "--miniprogram-diff-mode":
                options.miniProgramDiffMode = args[++i];
                break;
            case "--miniprogram-diff-reference":
                options.miniProgramDiffReference = args[++i];
                break;
            case "--miniprogram-diff-pages":
                options.miniProgramDiffPages = args[++i].split(",");
                break;
            case "--miniprogram-diff-max-pages":
                options.miniProgramDiffMaxPages = parseInt(args[++i], 10);
                break;
            case "--miniprogram-diff-threshold-pixels":
                options.miniProgramDiffThresholdPixels = parseInt(args[++i], 10);
                break;
            case "--miniprogram-diff-threshold-ratio":
                options.miniProgramDiffThresholdRatio = parseFloat(args[++i]);
                break;
            case "--build-index":
            ...
            case "--flaky-threshold-failure-rate":
                ...
                break;
            case "--flaky-threshold-flip-rate":
                options.flakyThresholdFlipRate = parseFloat(args[++i]);
                break;
            case "--flaky-min-runs":
                options.flakyMinRuns = parseInt(args[++i], 10);
                break;
            case "--mcp":
                options.mcp = true;
                break;
            ...
            case "--maestro":
                options.mobile = true;
                options.mobileTool = "maestro";
                break;
            case "--appium":
                options.mobile = true;
                options.mobileTool = "appium";
                break;
            case "--mobile-tool":
                options.mobileTool = args[++i];
                break;
            case "--mobile-page-health":
                options.mobilePageHealth = true;
                break;
            case "--mobile-app-id":
                options.mobileAppId = args[++i];
                break;
            case "--mobile-routes":
                options.mobileRoutes = args[++i].split(",");
                break;
            case "--mobile-screenshot-dir":
                options.mobileScreenshotDir = args[++i];
                break;
            case "--mobile-update-baseline":
                options.mobileUpdateBaseline = true;
                break;
            case "--mobile-concurrency":
                options.mobileConcurrency = parseInt(args[++i], 10) || 1;
                break;
            case "--mobile-timeout":
                options.mobileTimeout = parseInt(args[++i], 10) || 60000;
                break;
            // v3.15.0
            case "--profile":
    ...
    if (options.pageHealth) {
        ...
        try {
            const result = await runPageHealthCheck({
                ...
                metrics: options.pageHealthMetrics,
    ...
    // v3.11.1: 小程序自动化测试（微信/支付宝/抖音，支持多平台）
    if (options.miniProgram) {
        console.log(pc.cyan("🛰️  正在执行小程序自动化测试..."));
        console.log(pc.gray(`   平台: ${options.miniProgram}`));
        console.log("");

        try {
            const miniProgramInput = options.miniProgram;
            const testOptions = {
                projectDir: options.projectDir,
                screenshot: options.miniProgramScreenshot,
                updateBaseline: options.miniProgramUpdateBaseline,
                concurrency: options.pageHealthConcurrency,
                performance: options.miniProgramPerformance,
                performanceThresholds: options.miniProgramPerformanceThresholds,
                // v3.12.0
                crossPlatformDiff: options.miniProgramCrossPlatformDiff,
                diffMode: options.miniProgramDiffMode,
                diffReferencePlatform: options.miniProgramDiffReference,
                diffPages: options.miniProgramDiffPages,
                diffMaxPages: options.miniProgramDiffMaxPages,
                diffThresholdPixels: options.miniProgramDiffThresholdPixels,
                diffThresholdRatio: options.miniProgramDiffThresholdRatio,
            };
            if (miniProgramInput.includes(",")) {
                const platforms = miniProgramInput
                    .split(",")
                    .map((s) => s.trim())
                    .filter(Boolean);
                Object.assign(testOptions, { platforms });
            } else {
                Object.assign(testOptions, { platform: miniProgramInput });
            }

            const result = await runMiniProgramTest(testOptions);

            if (options.json) {
                console.log(JSON.stringify(formatMiniProgramJson(result), null, 2));
            } else {
                console.log(formatMiniProgramReport(result));
                console.log("");
            ...
            if (options.server) {
                ...
                const uploadResult = await uploadMiniProgramResult(result, dashboardConfig);
                if (!options.json) {
                    if (uploadResult.success) {
                        console.log(pc.green(`   ✅ 已上报到看板服务器`));
                    } else {
            ...
            console.error(pc.red("❌ 小程序自动化测试失败:"), err instanceof Error ? err.message : String(err));
            process.exit(1);
        }
    }

    // v4.0.0: 移动端测试（Maestro / Appium）
    if (options.mobile) {
        const integrations = [
            { name: "Maestro", integration: maestroIntegration, tool: "maestro" },
            { name: "Appium", integration: appiumIntegration, tool: "appium" },
        ];

        let selected;
        if (options.mobileTool === "auto") {
            selected = integrations.find((t) => t.integration.isAvailable(options.projectDir));
        } else {
            selected = integrations.find((t) => t.tool === options.mobileTool);
            if (selected && !selected.integration.isAvailable(options.projectDir)) {
                console.log(pc.yellow(`⚠️  未检测到 ${selected.name} 配置或依赖`));
                process.exit(1);
            }
        ...
        if (!selected) {
            if (options.mobileTool === "auto") {
                console.log(pc.yellow("⚠️  未检测到任何支持的移动端测试工具（Maestro / Appium）"));
            } else {
                console.log(pc.yellow(`⚠️  未知的移动端工具: ${options.mobileTool}`));
            }
            console.log(pc.gray("   支持的工具: auto | maestro | appium"));
            process.exit(1);
        }

        console.log(pc.cyan(`📱 正在运行 ${selected.name} 移动端测试...`));
        console.log(pc.gray("   这可能需要几分钟（取决于测试数量和模拟器启动时间）"));
        console.log("");
    ...
    // v4.0.0: 移动端页面健康检查
    if (options.mobilePageHealth) {
        console.log(pc.cyan("📱 正在执行移动端页面健康检查..."));
        console.log(pc.gray(`   工具: ${options.mobileTool}`));
        console.log("");

        try {
            const result = await runMobilePageHealthCheck({
                projectDir: options.projectDir,
                tool: options.mobileTool,
                appId: options.mobileAppId,
                routes: options.mobileRoutes,
                screenshot: options.mobileScreenshot,
                screenshotDir: options.mobileScreenshotDir,
                updateBaseline: options.mobileUpdateBaseline,
                concurrency: options.mobileConcurrency,
                timeout: options.mobileTimeout,
            });

            if (options.json) {
                console.log(JSON.stringify(formatMobilePageHealthJson(result), null, 2));
            } else {
                console.log(formatMobilePageHealthReport(result));
                console.log("");
                ...
                if (result.issues.length > 0) {
                    ...
                    console.log(pc.green("✅ 移动端页面检查通过"));
                }
            }
            ...
            console.error(pc.red("❌ 移动端页面健康检查失败:"), err instanceof Error ? err.message : String(err));
            process.exit(1);
        }
    }
    ...
    if (options.recommendTests) {
        ...
        const result = await recommendTests({
            ...
            minPriority: 1,
            flakyThresholds: {
                failureRate: options.flakyThresholdFailureRate,
...
async function runAllModules(options, cacheInstance) {
    ...
    if (options.compliance) {
        ...
        const _markdown = complianceReportToMarkdown(report);
    ...
    if (dashboardUrl) {
        const _allIssues = collectAllIssues(allResults, externalResults);
